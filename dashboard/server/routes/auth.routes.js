...
export function createAuthRouter({ services }) {
	...
	router.post(
		'/auth/group-admin-login',
		otpRateLimit,
		validate({ body: z.object({ phoneNumber: z.string().min(5) }) }),
		async (req, res) => {
	...
	router.post(
		'/auth/group-admin-finalize',
		validate({ body: z.object({ requestId: z.string().min(10), requestKey: z.string().min(10) }) }),
		async (req, res) => {
