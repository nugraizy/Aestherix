...
const FLAGS = {
	...
	skipSub: { type: 'boolean' },
	enableVoip: { type: 'boolean' }
};
...
const HELP_TEXT = `
 ...
 ${chalk.yellow('Options')}
   ...
   --silent                 ${chalk.green('Not showing any logs in the meantime still respond for any commands.')}
   ...
   --reject-calls           ${chalk.green('Reject incoming call.')}
   ...
   --unlimited              ${chalk.green('Set commands limit to none.')}
   ...
   --profile                ${chalk.green('Log per-message dispatch latency at PROFILE level.')}
    --skip-sub               ${chalk.green('Do not auto-start persisted sub-bots on connect.')}
    --enable-voip            ${chalk.green('Enable VoIP calling features.')}
