...
import { checkBan } from './check-banned.js';
...
const handleGroupParticipantEvent = async (client, message, store) => {
	if (WebMessageInfoStubType.GROUP_PARTICIPANT_ADD === message.messageStubType) {
		message = await Context.from(JSON.parse(JSON.stringify(message)), client, store, false);
		await checkBan(client, message);
	}

	client.ev.emit('group.participants.update', message, client);
};
...
const handleGroupSettingsEvent = (client, message) => {
	client.ev.emit('group.settings.update', message, client);
};
...
const handleMiscEvent = (client, message) => {
	client.ev.emit('message.oversized', message, client);
};
