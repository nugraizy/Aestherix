	...
	async run({ from, message, query, args, sender, prettyNumber, groupMetadata }, client) {
		...
		if (args[1] === 'play') {
			...
			if (configuration.games.wordle.has(sender) && wordle.message) {
				return await client[botNum].reply({ groupMetadata, from, quoted: wordle.message }, 'You are already playing Wordle.');
			}
			...
			if (!configuration.games.wordle.has(sender)) {
				return await client[botNum].reply({ groupMetadata, from, quoted: message }, 'You are not playing Wordle.');
			}
			...
			await client[botNum].reply({ groupMetadata, from, quoted: message }, 'You have exited Wordle.');
		} else if (args[1] === 'info') {
