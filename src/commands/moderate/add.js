...
export default {
	...
	status: 'enable',
	async run(
		{ isAdmin, isBotAdmin, isOwner, from, query, mention, bodyQuoted, mediaData, message, adminGroups, groupMetadata },
		client
	) {
		if (!isAdmin && !isOwner) {
		...
		if (!query && !bodyQuoted) {
			return await client[botNum].reply(
				{ groupMetadata, from, quoted: message },
				'Please reply people message or reply people'
			);
		}
		...
		if (
			...
			return await client[botNum].reply({ groupMetadata, from, quoted: message }, 'You can not add me by myself.');
		}
		...
		if (query) {
			if (mention.length > 0) {
				return await client[botNum].reply(
					{ groupMetadata, from, quoted: message },
					'Please reply people message or input people number.'
				);
			}
