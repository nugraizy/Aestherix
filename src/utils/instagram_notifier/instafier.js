...
class Instafier {
	...
	async saveState() {
		if (!fs.existsSync('./src/helper/connection/session/instagram_auth/')) {
			fs.mkdirSync('./src/helper/connection/session/instagram_auth/');
		}

		return fs.writeFileSync(
			`./src/helper/connection/session/instagram_auth/${process.env.INSTAGRAM_USERNAME}.json`,
	...
	async readState() {
		if (!fs.existsSync(`./src/helper/connection/session/instagram_auth/${process.env.INSTAGRAM_USERNAME}.json`)) {
			await this.login();
			await this.saveState();
		} else {
			await this.Instagram.importState(
				fs.readFileSync(`./src/helper/connection/session/instagram_auth/${process.env.INSTAGRAM_USERNAME}.json`, {
					encoding: 'utf8'
				})
			);
		}
