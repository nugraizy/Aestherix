...
const loadBaileys = async () => {
	...
	_baileysCache = {
		...
		decodeBinaryNode: b.decodeBinaryNode,
		encodeBinaryNode: b.encodeBinaryNode,
		getBinaryNodeChild: b.getBinaryNodeChild,
		getAllBinaryNodeChildren: b.getAllBinaryNodeChildren,
		encodeWAMessage: b.encodeWAMessage,
		unpadRandomMax16: b.unpadRandomMax16,
		parseAndInjectE2ESessions: b.parseAndInjectE2ESessions,
...
export class SignalingBridge {
	...
	queryCallLink = async (token, media = 'video') => {
		const stanza = {
			tag: 'call',
			attrs: { to: 'call', id: `link-query-${Date.now()}` },
			content: [{ tag: 'link_query', attrs: { media, token } }]
		};

		return this.#sock.query(stanza);
	};

	joinCallLink = async (token, media = 'video') => {
		const joinContent = [
		...
		const stanza = {
			tag: 'call',
			attrs: { to: 'call', id: `link-join-${Date.now()}` },
	...
	#sendCallStanza = async (routeTo, voipNode, signalingTag, effectivePeerJid, callbackPeerJid) => {
		...
		void (async () => {
			try {
				...
				} catch { /* ack dispatch failed */ }
			} catch { /* ack wait failed */ }
		})();
	};
	...
	#doProcessIncomingCall = async (node, voip, activeCallId) => {
		...
		let mappedRemoteDeviceJid = callIdForRouting
			? this.#remoteDevicePeerByCallId.get(callIdForRouting)
			: undefined;
	...
	#toBareJid = (jid) => {
		...
		const server = decoded.server === 'lid' || decoded.server === 'hosted.lid'
			? decoded.server
			: 's.whatsapp.net';
