...
import { CallSignaling } from './signaling.js';
...
export class ActiveCall extends EventEmitter {
	...
	_startHeartbeat = () => {
		console.log('[VOIP DEBUG] _startHeartbeat called, _callCreator:', this._callCreator, 'sendHeartbeat:', typeof this._voipClient?.sendHeartbeat);
		this.#maybeStartHeartbeat();
	};
	...
	#maybeStartHeartbeat = () => {
		...
		const tick = () => {
			try {
				...
				console.log('[VOIP DEBUG] heartbeat:', realCallId, realCreator);
...
export class VoipClient {
	...
	init = async () => {
		...
		this.#signaling = new CallSignaling({ sock: this.#sock });
	...
	#hookWsListeners = () => {
		...
		this.#sock.ws.on('CB:call', (node) => {
			this.#signaling?.handleIncomingCall(node, this.#engine, this.#activeCall?.callId ?? '');
		});
		this.#sock.ws.on('CB:receipt', (node) => {
			...
			this.#signaling?.handleIncomingReceipt(node, this.#engine, this.#activeCall?.callId ?? '');
		});
	};
	...
	joinLink = async (token, opts = {}) => {
		...
		console.log('[VOIP DEBUG] joinLink token:', token);
		...
		try {
			console.log('[VOIP DEBUG] joinLink calling previewAndJoinCallLink');
			this.#engine.previewAndJoinCallLink(token, !!opts.video, 0, '');
			console.log('[VOIP DEBUG] joinLink previewAndJoinCallLink returned');
		} catch (err) {
			console.log('[VOIP DEBUG] joinLink error:', err?.message);
			this.#activeCall = null;
			throw err;
		}
	...
	#handleCallEvent = (eventType, eventData) => {
		console.log('[VOIP DEBUG] onCallEvent:', eventType, typeof eventData === 'string' ? eventData?.slice(0, 200) : eventData);
		...
		if (eventType === 16 && eventData) {
			try {
				...
				console.log('[VOIP DEBUG] callState:', callState, 'activeCall:', this.#activeCall?.callId);
			...
			console.log('[VOIP DEBUG] call ended event type:', eventType);
