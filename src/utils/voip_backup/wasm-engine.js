...
const filterWorkerStderr = (chunk) => {
	...
	if (
		line &&
		!line.startsWith('voip:') &&
		!line.startsWith('still waiting') &&
		!line.includes('Blocking on the main thread')
	) {
		process.stderr.write(chunk);
	}
};

const BLOCKING_WARN_PATTERN = 'Blocking on the main thread';

const patchStderrFilter = () => {
	...
	process.stderr.write = function patched(chunk, ...rest) {
		...
		if (typeof chunk === 'string' && chunk.includes(BLOCKING_WARN_PATTERN)) {
			return true;
		}

		if (Buffer.isBuffer(chunk) && chunk.toString().includes(BLOCKING_WARN_PATTERN)) {
			return true;
		}
...
const resolveWorkerScriptPath = () => {
	return path.join(_voipDirname, 'worker-bootstrap.js');
};
...
class NodeWorkerMessagePort {
	...
	constructor(worker, name = 'WAWebVoipWebWasmWorker') {
		...
		this.fullyConnected.catch(() => { /* non-critical */ });

		if (typeof worker.on === 'function') {
			worker.on('message', (data) => this.#handleMessage(data));
			worker.on('error', () => { /* non-critical */ });
...
class WasmEngine {
	...
	constructor(config = {}) {
		...
		const wasmPath = config.wasmPath
			...
			: path.join(_voipDirname, 'whatsapp.wasm');
	...
	initialize = async () => {
		...
		const loaderFile = path.join(this.#config.resourcesPath, 'loader.js');
		const workerFile = path.join(this.#config.resourcesPath, 'worker-modules.js');
	...
	waitForVoipStackReady = async () => {
		if (this.#voipStackInitialized) {
			if (this.#voipStackInitError) {
				throw this.#voipStackInitError;
			}
	...
	startGroupCall = (options) => {
		...
		const pnUserJids = this.#makeStringList(options.participants);
		const lidUserJids = this.#makeStringList([]);
		const deviceJidsCsv = this.#makeStringList([]);
		const tcToken = options.extraData ? this.#createUint8List(options.extraData) : null;
		const isVideo = !!options.isVideo;

		try {
			const groupFn = this.#instance.startVoipGroupCall;
			...
			const groupFnLegacy = this.#instance.startGroupCall;

			if (typeof groupFnLegacy === 'function') {
				const callCreator =
					options.callCreator ||
					options.participants[0] ||
					'';

				return groupFnLegacy.call(
					this.#instance,
					options.callId ?? '',
					pnUserJids,
					isVideo,
			...
			const isLid = firstPeer?.includes('@lid');
	...
	setOnVideoFrameCallback = (cb) => {
		this.#ensureInitialized();
		this.#h264FrameCallback = cb;
		const setFn = this.#instance.setVideoFrameCallback;

		if (typeof setFn !== 'function') {
			return false;
		}

		setFn.call(this.#instance, (raw) => {
			cb({
				format: 'rgba',
				data: Buffer.from(raw.data ?? new Uint8Array(0)),
				timestamp: raw.timestamp ?? 0,
				width: raw.codedWidth ?? 0,
				height: raw.codedHeight ?? 0,
			});
		});
	...
	#h264FrameCallback = null;

	_onH264Packet = (nalu, timestamp, isKeyframe) => {
		if (!this.#h264FrameCallback) {
			return;
		}
	...
	getCallInfo = () => {
		...
		if (typeof this.#instance.getCallInfo !== 'function') {
			return null;
		}

		const raw = this.#instance.getCallInfo();
	...
	previewCallLink = (token, videoEnabled = false, lobbyEntryType = 0, username = '') => {
		...
		return this.#instance.previewCallLink(
			String(token ?? ''),
			!!videoEnabled,
			Number(lobbyEntryType ?? 0),
			String(username ?? ''),
		);
	};
	...
	joinCallLink = () => {
		...
		console.log('[VOIP DEBUG] joinCallLink typeof:', typeof this.#instance.joinCallLink);
	...
	previewAndJoinCallLink = (token, videoEnabled = false, lobbyEntryType = 0, username = '') => {
		this.#ensureInitialized();

		console.log('[VOIP DEBUG] previewAndJoinCallLink typeof:', typeof this.#instance.previewAndJoinCallLink);

		if (typeof this.#instance.previewAndJoinCallLink !== 'function') {
			return null;
		}

		console.log('[VOIP DEBUG] calling previewAndJoinCallLink with token:', token);
		const result = this.#instance.previewAndJoinCallLink(
			String(token ?? ''),
			!!videoEnabled,
			Number(lobbyEntryType ?? 0),
			String(username ?? ''),
		);
		console.log('[VOIP DEBUG] previewAndJoinCallLink returned:', result);
		...
		const preDefinedModules = {
			...
			WorkerBundleResource: {
				createDedicatedWebWorker: (resource) => {
					const scriptPath =
						resource?.resourcePath && fs.existsSync(resource.resourcePath) ? resource.resourcePath : resolveWorkerScriptPath();
...
export { WasmEngine, NodeWorkerMessagePort };
