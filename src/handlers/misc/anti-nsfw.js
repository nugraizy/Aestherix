...
const handler = async (
	...
	if (isBotAdmin && isMediaImage && isGroup && settings?.[from]?.antiNSFW === 'enable' && !configuration.OPTIONS.onlyLogs) {
		const filePath = path.join(__dirname, `src/media/temporary_files/${filename}.${extractMediaData.mimetype.split('/')[1]}`);
		...
		if (isAdmin) {
			return await client[botNum].reply({ groupMetadata, from, quoted: message }, JSON.stringify(check, undefined, 2));
		}
