...
export const checkBan = async (client, { from, isBotAdmin, isGroup, messageStubParameters, participant: admin }, settings) => {
	if (isGroup) {
		...
		for (const participant of messageStubParameters) {
			...
			if (isBanned && isBotAdmin) {
				await client[botNum].sendMessage(from, {
					text: `You were adding someone who is banned from this group (@${participant.split("@")[0]}). Are you sure you want to add him?`,
