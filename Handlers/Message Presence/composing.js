...
export default {
	async handler(client, from, participant) {
		if (checkAfk(participant, from)) {
			...
			await client[botNum].sendMessage(from, { text: `@${participant.split("@")[0]} detected writing. AFK since ${time} ago. Now they are out from AFK. Reason : ${reasons}`, contextInfo: { mentionedJid: [participant] } });
