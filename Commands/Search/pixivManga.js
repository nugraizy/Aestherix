	...
	async run({ from, query, message, cmd }, client) {
		...
		try {
			...
			for (const querie of queries) {
				...
				const container = [];
				let i = 0;
				const images = await (await fetch(dataImage.url.original[0], { headers: { referer: `https://www.pixiv.net/ajax/manga/${dataImage.id}` } })).arrayBuffer();
...
Total Media : ${dataImage.pageCount}`,
				...
				for (let j = 0; j < dataImage.url.original.length; j++) {
					if (j != 0) {
						const images = await (await fetch(dataImage.url.original[j], { headers: { referer: `https://www.pixiv.net/ajax/manga/${dataImage.id}` } })).arrayBuffer();
						await client[botNum].sendMessage(
							from,
							{
								image: new Buffer.from(images, "base64"),
