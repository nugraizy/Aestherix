...
const BASE_URL = (input) => `https://github.com/`;
...
${data[index].textMatches.map((v) => `_${v.texts}_\n\`\`\`${v.fragment}\`\`\``).join("\n")}`,
					templateButtons: [
						{ urlButton: { displayText: "User Avatar Source", url: args[1] == "next" ? data[index].owner.ownerPicture : data[index].owner.ownerPicture } },
						{ urlButton: { displayText: "Repository Source", url: args[1] == "next" ? data[index].repository.url : data[index].repository.url } },
						{ urlButton: { displayText: "Code Source", url: args[1] == "next" ? data[index].source : data[index].source } },
						index + 1 !== data.length ? { quickReplyButton: { displayText: "Next Code", id: `.githubcode next ${data[index + 1].source} ${JSON.stringify(data)}` } } : {},
						index !== 0 ? { quickReplyButton: { displayText: "Previous Code", id: `.githubcode prev ${data[index - 1].source} ${JSON.stringify(data)}` } } : {},
					],
...
${result[0].textMatches.map((v) => `_${v.texts}_\n\`\`\`${v.fragment}\`\`\``).join("\n")}`,
				templateButtons: [
					...
					result.length !== 1 ? { quickReplyButton: { displayText: "Next Code", id: `.githubcode next ${result[1].source} ${JSON.stringify(result).replace(/\|/g, "")}` } } : {},
				],
