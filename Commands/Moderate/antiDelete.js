	...
	async run(message, client) {
		...
		switch (message.query.toLowerCase()) {
			...
			case 'on':
				...
				await client[botNum].reply({ from: message.from, quoted: message.message }, 'You have successfully enabled anti-delete');
				break;
			case 'disable':
			case 'off':
				if (!isEnable) {
				...
				await client[botNum].reply({ from: message.from, quoted: message.message }, 'You have successfully disabled anti-delete');
				break;
			default:
