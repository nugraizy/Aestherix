...
export default {
	...
	async run(message, client, store) {
		...
		if (message.type == "buttonsResponseMessage") {
			return await client[botNum].updateGroup(message.from, message.mention.length > 0 ? message.mention : message.query.split(",").parse(), "REMOVE", false, /--?(force|F)/.test(message.query), message.message);
		} else if (!message.query && message.mention.length == 0 && !message.bodyQuoted) return client[botNum].reply({ from: message.from, quoted: message.message }, "Please reply people message or mention people.");
