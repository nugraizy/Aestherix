...
export default {
	...
	async run(message, client, args) {
		...
		const { isOne, isURL, isEmpty, isSame, numberWithCommas } = await import("../../Helper/Modules/functions.js");
		...
		for (const username of usernames) {
			...
			if ("error" in story) client[botNum].reply(message.from, story.error);
			else {
				...
				if (isOne(story.medias.length))
					...
					for (let j = 0; j < story.medias.length; j++) {
						client[botNum].sendMessage(message.from, isSame(story.medias[j].type, "video") ? { video: { url: story.medias[j].url } } : { image: { url: story.medias[j].url } });
						await delay(300);
					}
				}
			}
