...
export default {
	...
	async run({ from }, client) {
		...
		if (typeof stopping == "boolean" && !stopping) {
			return await client[botNum].reply(from, "You are not in a search!");
		}
		if (typeof stopping == "object" && stopping.partner2) {
			await client[botNum].reply(stopping.partner1, "You've stopped the chat!");
			await client[botNum].sendMessage(stopping.partner2, { text: "Your partner stoped the chat!" });
		} else {
			await client[botNum].reply(from, `You already searching for a partner!\nPlease wait for ${stopping.seconds}s`);
		}
	},
};
