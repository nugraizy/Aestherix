export default {
	...
	async run({ isOwner, from, query, message }, client) {
		...
		if (typeof client[botNum].updateProfileName !== "function") {
			return await client[botNum].reply({ from, quoted: message }, "Your current Baileys didn't support changing profile name, please update to latest commit of the Baileys.");
		}
