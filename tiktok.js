...
const API_BASE_URL = 'https://api-t2.tiktokv.com/';
...
class ResponseParser {
	 ...
	 * @private
	 */
	_apiBase = (input) => `https://www.tiktok.com/${input}`;

	/**
	 * @private
	 */
	_apiBaseVideo = (...input) => this._apiBase(`@${input[0]}/video/${input[1]}`); // eslint-disable-line
		...
		const data =
			Object.keys(arr?.ItemModule || []).length === 0
				...
				: Object.values(arr.ItemModule).map((v) => ({
						...
						url: {
							sourceUrl: this._apiBaseVideo(arr.UserPage.uniqueId, v.id),
							music: {
								title: v.music.title,
	...
	_getRequestConfig() {
		return {
			headers: {
				...
				Cookie: process.env.COOKIE_TIKTOK_COM
			}
		};
	}
...
console.log(await tiktok.downloadMedia('https://vt.tiktok.com/ZSNjDD8BN/'));
