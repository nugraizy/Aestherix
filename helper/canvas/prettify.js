...
GlobalFonts.registerFromPath(path.join('.', 'media_files/fonts/zh-cn.ttf'), 'ZHCN');
...
export class Prettify {
	...
	constructor() {
		this.Carbon = (code, options) => this.#screenshotNow(this.#carbonNow(code, options).toBuffer(), true);
		this.Screenshot = (buffer, watermark) => this.#screenshotNow(buffer, false, watermark);
	}
	...
	async #screenshotNow(buffer, isCarbon, watermark = 'Prettify Screenshot by Hidden Finder') {
		...
		return { toBuffer: () => this.#toBuffer(canvas) };
	}
	...
	#carbonNow(code, options) {
		const theme = Object.assign(this.defaultCarbonOptions, options);
		...
		this.#fillText(ctx, x, y, fontSize, lineHeight, parsed, palette);
	...
	#fillText(ctx, x, y, fontSize, lineHeight, parentNode, colors) {
		let multiply = 0;
		let index = 0;
