...
if command -v xvfb-run &>/dev/null; then
  exec xvfb-run -a \
    ...
    node . nugraizy_android --watch --pair-mode --limit-reset --multi-cmd --cool-down --pipe
else
  echo "[WARN] xvfb-run not found. WebGL mesh gradients will use SVG fallback."
  echo "       Install with: sudo apt install xvfb"
  exec node . nugraizy_android --watch --pair-mode --limit-reset --multi-cmd --cool-down --pipe
fi
