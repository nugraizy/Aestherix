...
export default {
	...
	async run({ from, query, prettyNumber, message }, client) {
		...
		for (const username of usernames) {
			...
			if ('error' in user) {
				await client[botNum].reply({ from, quoted: message }, `Error while searching Twitter user\n\n${user.error}\n${username}`);

				ERRLOG(`[${color(time, 'cyan')}]`, `⚠️ ${color('Failed to Searching Twitter User', 'red')} for ${color(prettyNumber, '#ff71ce')}`);
