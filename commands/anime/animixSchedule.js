	...
	run: async ({ from, message, type, cmd, args }, client) => {
		...
		if (type == 'listResponseMessage') {
			...
			client[botNum].reply(
				{ from, quoted: message },
				`${text}\n\nStream Here\n\n${data
					.map((v) => `📁 ${v.server}\n${v.items.map((w, i) => ` ╠  📂${w.title}\n ${i === v.items.length - 1 ? '╚' : '╠'}  📂 ${w.url}`).join('\n')}`)
					.join('\n\n')}`.trim(),
			);
		...
		for (const day in result) {
			container.push({
				rows: [
					{ title: `${result[day][0].title}`, description: `Releases : ${result[day][0].time}`, rowId: `${cmd} ${JSON.stringify(result[day][0].streams).replace(/\|/g, '')}` },
					...result[day].slice(1).map((v) => ({ title: `${v.title}`, description: `Releases : ${v.time}`, rowId: `${cmd} ${JSON.stringify(v.streams).replace(/\|/g, '')}` })),
				],
