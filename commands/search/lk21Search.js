...
Void Bot     ${index + 1}/${data.length}\nPowered by 𓆩 𝚮ɪᴅᴅᴇɴ 𝐅ɪɴᴅᴇʀ ⁣𓆪`,
					templateButtons: [
						index + 1 !== data.length ? { quickReplyButton: { displayText: 'Next Movie', id: `.layarkaca21 next ${data[index + 1].source} ${JSON.stringify(data)}` } } : {},
						index !== 0 ? { quickReplyButton: { displayText: 'Previous Movie', id: `.layarkaca21 prev ${data[index - 1].source} ${JSON.stringify(data)}` } } : {},
					],
				},
				{ quoted: message },
			);
...
Void Bot     1/${result.length}\nPowered by 𓆩 𝚮ɪᴅᴅᴇɴ 𝐅ɪɴᴅᴇʀ ⁣𓆪`,
				templateButtons: [result.length !== 1 ? { quickReplyButton: { displayText: 'Next Movie', id: `.layarkaca21 next ${result[1].source} ${JSON.stringify(result)}` } } : {}],
			},
			{ quoted: message },
		);
	},
