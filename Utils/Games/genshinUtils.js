...
const getDS = (server, { query, body }) => {
	return server == "os" ? getOS_DSKey() : getCN_DSKey({ query, body });
};
...
const getCN_DSKey = ({ query, body }) => {
	const time = Math.floor(Date.now() / 1000);
	const random = Math.floor(Math.random() * (200_000 - 100_000 + 1)) + 100_000;
	return `${time},${random},${Crypto.createHash("md5")
		.update(`salt=${SALT["cn"]}&t=${time}&r=${random}&b=${body ? JSON.stringify(sortKeys(body)) : ""}&q=${query ? new url.URLSearchParams(sortKeys(query)) : ""}`)
		.digest("hex")}`;
};
