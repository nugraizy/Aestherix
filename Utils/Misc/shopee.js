...
const API_URL = (product, total) => `https://shopee.co.id/api/v4/search/search_items?by=relevancy&keyword=${product}&limit=${total}&newest=0&order=desc&page_type=search&scenario=PAGE_GLOBAL_SEARCH&version=2`;
		...
		try {
			...
			DATA.data.items.forEach((element) => {
				CONTAINER.items.push({
					...
					brand: element.item_basic.brand,
					...
					discountPercent: element.item_basic.discount,
