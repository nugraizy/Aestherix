...
const fetchId = async (usernames) => {
	...
	return {
		user: dataUser,
		items: data.data.user.edge_highlight_reels.edges.map((edge) => ({ highlightId: edge.node.id, cover: edge.node.cover_media.thumbnail_src, title: edge.node.title })),
	};
};
...
export const getHighlights2 = (username) =>
	new Promise(async (resolve, reject) => {
		try {
			...
			const Container = await Promise.all(items.map((v) => fetchHighlights(v.highlightId)));
