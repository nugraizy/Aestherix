	...
	new Promise(async (resolve, reject) => {
		try {
			...
			const userRaw = await fetchJSON(
				`https://api2.musical.ly/aweme/v1/discover/search/?keyword=${parsed.author}&cursor=0&count=10&type=1&hot_search=0&search_source=discover${new URLSearchParams(
			...
			const withNoWatermark = videoRaw.aweme_list.find((v) => v.aweme_id === parsed.keyword).video.play_addr.url_list[Math.floor(Math.random() * 3)];
